(function($) {
    ...
    function formatAndDecorate(config, data) {
        ...
        let props = getUnsortedProperties(data);
    ...
    // xavi 25-02-2024
    function getUnsortedProperties(obj) {
        let props = [];
        for (let prop in obj) {
            if (obj.hasOwnProperty(prop)) {
                props.push(prop);
        ...
        for (i = 0; i < diffs.length; i++) {
            ...
            for (let j = 0; j < linesLeft.length; j++) {
                if (diffs[i]['path1']['line'] == j + 1 && diffs[i]['type'] == 'eq' || diffs[i]['type'] == 'type') {
                    $(`#${blockLeftId}`).text(textLeft.replace(linesLeft[j], `<span style="color:#FF0000;font-weight: bold;">${linesLeft[j]}</span>`));
            ...
            for (let j = 0; j < linesRight.length; j++) {
                if (diffs[i]['path2']['line'] == j + 1 && diffs[i]['type'] == 'eq') {
                    $(`#${blockRightId}`).text(textRight.replace(linesRight[j], `<span style="color:#FF0000;font-weight: bold;">${linesRight[j]}</span>`));
                }
